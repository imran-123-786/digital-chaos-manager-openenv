...
def compute_grader_score(metrics: dict[str, Any]) -> dict[str, float]:
    ...
    completion_score = clamp01(completed_tasks / total_tasks)

    order_total = max(1, int(metrics.get("priority_events", 0)))
    order_violations = int(metrics.get("priority_violations", 0))
    priority_score = clamp01(1.0 - (order_violations / order_total))
    ...
    distraction_score = clamp01(distraction_raw)

    steps_used = max(1, int(metrics.get("steps_used", 0)))
    time_budget = max(1, int(metrics.get("time_budget", 0)))
    pace_score = clamp01((completed_tasks / steps_used) * (time_budget / max(steps_used, 1)))
    efficiency_score = clamp01(0.5 * completion_score + 0.5 * pace_score)
